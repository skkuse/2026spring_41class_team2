...
| [../db-schema/reviews-likes.md](../db-schema/reviews-likes.md) | `movie_bookmarks` 테이블과 RLS 기준 |
...
participant "Route Handler\n/api/me/movie-bookmarks/{movieId}" as Route
...
group 찜 추가
  User -> FE: 찜 버튼 클릭
  FE -> Route: PUT /api/me/movie-bookmarks/{movieId}
...
group 찜 삭제
  User -> FE: 찜 해제 클릭
  FE -> Route: DELETE /api/me/movie-bookmarks/{movieId}
...
actor "사용자" as User
participant "Frontend\n/mypage" as FE
participant "Route Handler\n/api/me/movie-bookmarks" as Route
participant "bookmark service" as Service
participant "repository" as Repo
database "Supabase Postgres" as DB

User -> FE: 찜한 영화 탭 진입
FE -> Route: GET /api/me/movie-bookmarks?page=&size=
...
`PUT /api/me/movie-bookmarks/{movieId}`는 로그인 사용자의 찜 목록에 영화를 추가한다.
...
`DELETE /api/me/movie-bookmarks/{movieId}`는 로그인 사용자의 찜 목록에서 영화를 삭제한다.

처리 기준:

- 인증되지 않은 요청은 `401 Unauthorized`를 반환한다.
- 존재하지 않는 영화에 대한 삭제는 `404 Not Found` 또는 멱등 성공 중 하나로 고정한다. 초기 구현은 추가 API와 일관되게 영화 존재 확인 후 `404 Not Found`를 반환한다.
...
`GET /api/me/movie-bookmarks`는 로그인 사용자의 찜한 영화 목록을 조회한다.
...
- 응답은 API 스펙에 맞춰 `movies: { id, title, year, posterUrl }[]`, `totalCount`를 반환한다.
...
| `app/api/me/movie-bookmarks/[movieId]/route.ts` | `PUT`, `DELETE` adapter |
...
| DB 제약/RLS 실패 | 공통 서버 에러 응답, 로그에는 requestId 포함 |

서비스 레벨에서는 RLS에만 의존하지 않고 `context.user.id` 조건을 명시적으로 사용한다.
...
4. `app/api/me/movie-bookmarks`와 `app/api/me/movie-bookmarks/[movieId]` Route Handler를 연결한다.
5. 영화 목록/상세/추천 응답에서 `isBookmarked` 계산을 공통 조회 방식으로 연결한다.
6. `/search`, `/movie/[id]`, `/recommend`의 찜 버튼을 API 기반으로 전환한다.
7. `/mypage`의 찜한 영화 탭을 `GET /api/me/movie-bookmarks`로 전환한다.
...
| 목록 조회 | `GET /api/me/movie-bookmarks`가 현재 사용자의 찜 영화만 반환한다. |
