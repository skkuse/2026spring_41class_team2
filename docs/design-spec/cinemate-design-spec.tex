...
\hypersetup{colorlinks=false,pdfborder={0 0 0},pdftitle={디자인 명세서},pdfauthor={Cinemate}}
...
\providecommand{\tightlist}{%
  \setlength{\itemsep}{0.25em}\setlength{\parskip}{0.1em}}
\renewcommand{\labelitemi}{-}
\renewcommand{\labelitemii}{-}
\setlist[itemize]{leftmargin=1.4em,itemsep=0.28em,topsep=0.25em,parsep=0.08em,label=\textbullet}
\setlist[enumerate]{leftmargin=1.8em,itemsep=0.30em,topsep=0.25em,parsep=0.08em}
\titleformat{\paragraph}[block]{\normalfont\normalsize\bfseries}{\theparagraph}{1em}{}
\titlespacing*{\paragraph}{0pt}{1.0em}{0.45em}
\titleformat{\subparagraph}[block]{\normalfont\normalsize\bfseries}{\thesubparagraph}{1em}{}
\titlespacing*{\subparagraph}{0pt}{0.9em}{0.4em}
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\nouppercase{\leftmark}}
\fancyhead[R]{\thepage}
...
{\large Document Date: 24 May, 2026\par}
...
\hypertarget{readership}{%
\subsection{Readership}\label{readership}}
...
\tightlist
\item
  Purpose: 본 문서의 목적, 예상 독자, 설계 범위 및 문서 구조를 설명한다.
\item
  Introduction: 본 문서에서 실제로 사용하는 다이어그램, 도구, 프로젝트 범위 및 참고 자료를 설명한다.
\item
  System Architecture - Overall: 씨네메이트의 전체 시스템 구조와 주요 구성 요소 간의 관계를 설명한다.
\item
  System Architecture - Frontend: 프론트엔드 화면 구조, 컴포넌트 구성, 상태 처리, API 연동 방식을 설명한다.
\item
  System Architecture - Backend: 백엔드 API 서버 구조와 주요 하위 시스템의 처리 흐름을 설명한다.
\item
  Protocol Design: 프론트엔드와 백엔드 사이의 통신 방식, 요청 및 응답 형식, API 목록을 설명한다.
\item
  Database Design: 씨네메이트에서 사용하는 주요 데이터와 ER Diagram, entity description을 설명한다.
\item
  Testing Plan: 씨네메이트의 개발 테스트, 릴리스 테스트, 사용자 테스트 계획을 설명한다.
\item
  Development Plan: 씨네메이트 구현에 사용되는 개발 환경, 기술, 제약사항 및 의존성을 설명한다.
\item
  Supporting Information: 문서 작성 기준과 문서 수정 이력을 설명한다.
\end{enumerate}
...
\hypertarget{applied-diagrams}{%
\subsection{Applied Diagrams}\label{applied-diagrams}}

\hypertarget{used-tools}{%
\subsubsection{Used Tools}\label{used-tools}}
...
\subsubsection{Sequence Diagram}\label{sequence-diagram}}
...
\hypertarget{system-architecture---overall}{%
\section{System Architecture - Overall}\label{system-architecture---overall}}
...
\subsection{System Organization}\label{system-organization}}
...
\tightlist
\item
  \textbf{프론트엔드}: Next.js 기반 사용자 화면을 제공한다. 홈, 검색, 영화 상세, 로그인, 온보딩, AI 추천 채팅, 캐릭터 대화, 맞춤 추천, 마이페이지 화면을 통해 사용자의 입력과 조회 요청을 처리한다.
\item
  \textbf{백엔드}: Next.js Route Handlers를 통해 인증, 영화 조회, 찜하기/리뷰, 맞춤 추천, AI 추천 채팅, 캐릭터 대화, 마이페이지 기능을 처리한다. 프론트엔드 요청을 받아 입력값과 권한을 확인하고, 필요한 데이터 조회 및 저장을 수행한다.
\item
  \textbf{데이터베이스}: Supabase DB에 사용자, 영화, 리뷰, 찜한 영화, 온보딩, 추천, 채팅 데이터를 저장한다. 맞춤 추천 결과는 사전 적재된 추천 데이터를 바탕으로 조회된다.
\item
  \textbf{외부 API 및 데이터}: Supabase Auth는 사용자 인증에 사용된다. TMDB API는 MovieLens 데이터에 부족한 영화 메타데이터를 보충하기 위해 사용된다. MovieLens 데이터셋은 추천 데이터 구성에 활용되며, OpenAI API는 AI 추천 채팅과 캐릭터 대화 응답 생성에 사용된다.
\end{itemize}
...
\hypertarget{system-architecture---frontend}{%
\section{System Architecture - Frontend}\label{system-architecture---frontend}}
...
\hypertarget{frontend-structure}{%
\subsection{Frontend Structure}\label{frontend-structure}}
...
\hypertarget{screen-organization}{%
\subsection{Screen Organization}\label{screen-organization}}
...
\hypertarget{public-screens}{%
\subsubsection{Public Screens}\label{public-screens}}
...
\hypertarget{authenticated-screens}{%
\subsubsection{Authenticated Screens}\label{authenticated-screens}}
...
\hypertarget{personalized-and-chat-screens}{%
\subsubsection{Personalized and Chat Screens}\label{personalized-and-chat-screens}}
...
\hypertarget{component-structure}{%
...
\hypertarget{state-and-api-interaction}{%
\subsection{State and API Interaction}\label{state-and-api-interaction}}
...
\hypertarget{navigation-flow}{%
\subsection{Navigation Flow}\label{navigation-flow}}
...
\hypertarget{system-architecture---backend}{%
\section{System Architecture - Backend}\label{system-architecture---backend}}
...
\hypertarget{overall-backend-architecture}{%
\subsection{Overall Backend Architecture}\label{overall-backend-architecture}}
...
\tightlist
\item
  \textbf{Route Handler}: 프론트엔드의 HTTP 요청을 수신하고, 요청 파라미터와 body를 검증한다. 인증이 필요한 요청에서는 현재 사용자를 확인한 뒤 service를 호출한다.
\item
  \textbf{Service}: 백엔드 기능의 처리 흐름을 담당한다. 영화 조회, 리뷰 작성, 찜하기, 온보딩 저장, 맞춤 추천, AI 추천 채팅, 캐릭터 대화와 같은 유스케이스를 조립한다.
\item
  \textbf{Repository}: Supabase DB에 저장된 영화, 사용자, 리뷰, 추천, 채팅 데이터를 조회하거나 저장한다.
\item
  \textbf{External API}: Supabase Auth는 사용자 인증을 처리하고, OpenAI API는 AI 추천 채팅과 캐릭터 대화 응답 생성에 사용된다.
\end{itemize}
...
\tightlist
\item
  사용자가 프론트엔드 화면에서 요청을 수행한다.
\item
  프론트엔드는 Next.js API Route로 HTTP 요청을 보낸다.
\item
  Route Handler는 요청값과 인증 상태를 확인한다.
\item
  Service는 요청 목적에 맞는 비즈니스 흐름을 수행한다.
\item
  Repository 또는 외부 API를 통해 필요한 데이터를 조회하거나 저장한다.
\item
  처리 결과를 JSON 응답으로 프론트엔드에 반환한다.
\end{enumerate}

\clearpage

\hypertarget{subcomponents}{%
\subsection{Subcomponents}\label{subcomponents}}

\hypertarget{user-management-system}{%
\subsubsection{User Management System}\label{user-management-system}}
...
\paragraph{Responsibilities}\label{responsibilities}}
...
\paragraph{Sequence Diagram}\label{sequence-diagram-1}}
...
\hypertarget{movie-information-system}{%
\subsubsection{Movie Information System}\label{movie-information-system}}
...
\hypertarget{responsibilities-1}{%
\paragraph{Responsibilities}\label{responsibilities-1}}
...
\paragraph{Sequence Diagram}\label{sequence-diagram-2}}
...
\hypertarget{review-and-favorite-system}{%
\subsubsection{Review and Favorite System}\label{review-and-favorite-system}}
...
\hypertarget{responsibilities-2}{%
\paragraph{Responsibilities}\label{responsibilities-2}}
...
\hypertarget{sequence-diagram-3}{%
\paragraph{Sequence Diagram}\label{sequence-diagram-3}}
...
\hypertarget{onboarding-and-recommendation-system}{%
\subsubsection{Onboarding and Recommendation System}\label{onboarding-and-recommendation-system}}
...
\hypertarget{responsibilities-3}{%
\paragraph{Responsibilities}\label{responsibilities-3}}
...
\hypertarget{sequence-diagram-4}{%
\paragraph{Sequence Diagram}\label{sequence-diagram-4}}
...
\hypertarget{ai-recommendation-chat-system}{%
\subsubsection{AI Recommendation Chat System}\label{ai-recommendation-chat-system}}
...
\hypertarget{responsibilities-4}{%
\paragraph{Responsibilities}\label{responsibilities-4}}
...
\hypertarget{sequence-diagram-5}{%
\paragraph{Sequence Diagram}\label{sequence-diagram-5}}
...
\hypertarget{character-conversation-system}{%
\subsubsection{Character Conversation System}\label{character-conversation-system}}
...
\hypertarget{responsibilities-5}{%
\paragraph{Responsibilities}\label{responsibilities-5}}
...
\hypertarget{sequence-diagram-6}{%
\paragraph{Sequence Diagram}\label{sequence-diagram-6}}
...
\section{Protocol Design}\label{protocol-design}}
...
\hypertarget{request-and-response-format}{%
\subsection{Request and Response Format}\label{request-and-response-format}}
...
\hypertarget{api-list}{%
\subsection{API List}\label{api-list}}

씨네메이트의 API는 기능 영역에 따라 사용자, 영화, 리뷰와 찜하기, 온보딩과 맞춤 추천, AI 추천 채팅, 캐릭터 대화 API로 구분된다.

\hypertarget{user-api}{%
\subsubsection{User API}\label{user-api}}
...
\hypertarget{movie-api}{%
\subsubsection{Movie API}\label{movie-api}}
...
\hypertarget{review-and-favorite-api}{%
\subsubsection{Review and Favorite API}\label{review-and-favorite-api}}
...
\hypertarget{onboarding-and-recommendation-api}{%
\subsubsection{Onboarding and Recommendation API}\label{onboarding-and-recommendation-api}}
...
\hypertarget{ai-recommendation-chat-api}{%
\subsubsection{AI Recommendation Chat API}\label{ai-recommendation-chat-api}}
...
\hypertarget{character-conversation-api}{%
\subsubsection{Character Conversation API}\label{character-conversation-api}}
...
\hypertarget{er-diagram}{%
\subsection{ER Diagram}\label{er-diagram}}

ERD는 영화 장르 및 인물 정보, 사용자 활동, 맞춤 추천, AI 추천 채팅, 캐릭터 대화 영역으로 구분하여 설명한다.

\hypertarget{genre-and-people}{%
\subsubsection{Genre and People}\label{genre-and-people}}
...
\tightlist
\item
  \texttt{movies}: 영화 기본 정보
\item
  \texttt{genres}: TMDB 장르 정보
\item
  \texttt{movie\_genres}: 영화와 장르의 연결 정보
\item
  \texttt{people}: 배우 및 제작진 인물 정보
\item
  \texttt{movie\_casts}: 영화 출연진과 배역 정보
\item
  \texttt{movie\_crew}: 영화 제작진 정보
\end{itemize}

\hypertarget{review-and-bookmark}{%
\subsubsection{Review and Bookmark}\label{review-and-bookmark}}
...
\tightlist
\item
  \texttt{profiles}: 사용자 프로필 정보
\item
  \texttt{movies}: 영화 기본 정보
\item
  \texttt{movie\_bookmarks}: 사용자와 찜한 영화의 관계
\item
  \texttt{reviews}: 영화 리뷰, 평점, 작성자 정보
\item
  \texttt{review\_likes}: 리뷰 좋아요 정보
\end{itemize}

\hypertarget{onboarding-and-recommendation}{%
\subsubsection{Onboarding and Recommendation}\label{onboarding-and-recommendation}}
...
\item
  \texttt{movies}: 영화 기본 정보
\item
  \texttt{user\_onboarding\_movies}: 사용자가 온보딩에서 선택한 선호 영화
\item
  \texttt{movie\_stats}: MovieLens 평점, 씨네메이트 리뷰 집계 등 영화 통계
\item
  \texttt{movie\_similarities}: Item CF 기반 영화 간 유사도
\item
  \texttt{movie\_tags}: MovieLens Tag Genome 태그
\item
  \texttt{movie\_tag\_relevances}: 영화와 태그의 관련도
\item
  \texttt{movie\_tag\_mapping\_embeddings}: 태그 매핑용 embedding 정보
\end{itemize}

\hypertarget{ai-recommendation-chat}{%
\subsubsection{AI Recommendation Chat}\label{ai-recommendation-chat}}
...
\item
  \texttt{movies}: 영화 기본 정보
\item
  \texttt{recommendation\_chat\_conversations}: 사용자별 AI 추천 채팅 대화
\item
  \texttt{recommendation\_chat\_conversation\_messages}: 대화 메시지와 분석 결과
\item
  \texttt{recommendation\_chat\_conversation\_message\_movies}: 응답 메시지에 포함된 추천 영화와 추천 사유
\end{itemize}

\hypertarget{character-chat}{%
\subsubsection{Character Chat}\label{character-chat}}
...
\item
  \texttt{profiles}: 사용자 프로필 정보
\item
  \texttt{characters}: 영화별 캐릭터 정보, 설명, 첫 인사, persona prompt, 캐릭터 이미지 경로
\item
  \texttt{character\_chat\_events}: 영화 대본에서 추출한 주요 사건 요약
\item
  \texttt{character\_chat\_event\_participants}: 특정 사건을 각 캐릭터가 어떻게 겪고 기억하는지에 대한 정보
\item
  \texttt{character\_chat\_default\_questions}: 캐릭터 대화 시작 시 보여줄 기본 추천 질문
\item
  \texttt{character\_chat\_conversations}: 사용자와 캐릭터 사이의 대화 세션
\item
  \texttt{character\_chat\_conversation\_messages}: 캐릭터 대화 메시지 기록
\end{itemize}

\hypertarget{entity-descriptions}{%
\subsection{Entity Descriptions}\label{entity-descriptions}}

\hypertarget{genre-and-people-1}{%
\subsubsection{Genre and People}\label{genre-and-people-1}}
...
\hypertarget{review-and-bookmark-1}{%
\subsubsection{Review and Bookmark}\label{review-and-bookmark-1}}
...
\hypertarget{onboarding-and-recommendation-1}{%
\subsubsection{Onboarding and Recommendation}\label{onboarding-and-recommendation-1}}
...
\hypertarget{ai-recommendation-chat-1}{%
\subsubsection{AI Recommendation Chat}\label{ai-recommendation-chat-1}}
...
\hypertarget{character-chat-1}{%
\subsubsection{Character Chat}\label{character-chat-1}}
...
\hypertarget{testing-policy}{%
\subsection{Testing Policy}\label{testing-policy}}

\hypertarget{development-testing}{%
\subsubsection{Development Testing}\label{development-testing}}
...
\hypertarget{release-testing}{%
\subsubsection{Release Testing}\label{release-testing}}
...
\hypertarget{testing-case}{%
\subsubsection{Testing Case}\label{testing-case}}
...
\section{Development Plan}\label{development-plan}}
...
\hypertarget{frontend-environment}{%
\subsection{Frontend Environment}\label{frontend-environment}}

\hypertarget{next.js}{%
\subsubsection{Next.js}\label{next.js}}
...
\subsubsection{Tailwind CSS}\label{tailwind-css}}
...
\hypertarget{backend-environment}{%
\subsection{Backend Environment}\label{backend-environment}}

\hypertarget{next.js-route-handler}{%
\subsubsection{Next.js Route Handler}\label{next.js-route-handler}}
...
\hypertarget{supabase}{%
\subsubsection{Supabase}\label{supabase}}
...
\hypertarget{drizzle-orm}{%
\subsubsection{Drizzle ORM}\label{drizzle-orm}}
...
\hypertarget{openai-api}{%
\subsubsection{OpenAI API}\label{openai-api}}
...
\hypertarget{tmdb-api}{%
\subsubsection{TMDB API}\label{tmdb-api}}
...
\hypertarget{movielens-dataset}{%
\subsubsection{MovieLens Dataset}\label{movielens-dataset}}
...
\hypertarget{assumptions-and-dependencies}{%
\subsection{Assumptions and Dependencies}\label{assumptions-and-dependencies}}
...
\hypertarget{software-design-specification}{%
\subsection{Software Design Specification}\label{software-design-specification}}
